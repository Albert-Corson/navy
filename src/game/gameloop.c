...
int send_attack(char **his_board, int his_pid, int hit)
{
    ...
    return (receive_package(2));
}

int receive_attack(char **my_board, char **his_board, int his_pid)
{
    vector_t target;
    int hit = 0;
    ...
    hit = check_hit(my_board, his_board, target);
...
int gameloop(char **my_board, char**his_board, int his_pid, int turn)
{
    ...
    RETURN_IF(kill(his_pid, 0) == -1, my_puterror("Enemy disconnected\n"));
    if (turn) {
        hit = send_attack(his_board, his_pid, hit);
        RETURN_IF(hit == 84, 84);
    } else
        hit = receive_attack(my_board, his_board, his_pid);
